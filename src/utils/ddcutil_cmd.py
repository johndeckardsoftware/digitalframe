...
def get_vcp_value(code):
    try:
        result = subprocess.run(["ddcutil", "-t", "getvcp", code], capture_output=True, text=True)
        ret = result.stdout.split(" ")
        if len(ret) > 3 and ret[0] == "VCP" and ret[1] == code:
            value_type = ret[2]
            if value_type == "C":
                cur = int(ret[3])
                max = int(ret[4])
